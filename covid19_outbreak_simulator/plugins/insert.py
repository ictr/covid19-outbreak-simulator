        ...
        for ps in args.popsize:
            ...
            population.add(
                [
                    Individual(
                        ID,
                        susceptibility=getattr(
                            self.simulator.model.params, "susceptibility_mean", 1
                        )
                        * getattr(
                            self.simulator.model.params,
            ...
            infected_list = (
                f',Infected={",".join(infected)}' if args.verbosity > 1 else ""
            )
            if args.verbosity > 0:
                self.logger.write(
                    f'{time:.2f}\t{EventType.PLUGIN.name}\t.\tname=insert,subpop={name},size={sz},n_infected={n_infected},IDs={",".join(IDs)}{infected_list}\n'
                )
