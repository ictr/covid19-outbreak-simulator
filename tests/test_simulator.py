...
def test_option_susceptibility():
    ...
    # not float
    with pytest.raises(ValueError):
        args = parse_args(["--popsize", "A=500", "B=300", "--susceptibility", "A=1f"])
        params = Params(args)
...
def test_option_inclubation_period():
    ...
    args = parse_args(["--popsize", "A=500", "B=300", "--incubation-period", "A=0.8"])
    params = Params(args)
    assert sorted(list(params.groups.keys())) == ["A", "B"]
...
def test_option_infectors():
    ...
    with pytest.raises(ValueError):
        args = parse_args(["--popsize", "A=10", "B=10", "--infectors", "C_1", "C_2"])
        params = Params(args)
...
def test_option_prop_asym_carriers():
    ...
    # not a float number
    with pytest.raises(ValueError):
        args = parse_args(["--popsize", "A=10", "B=10", "--prop-asym-carriers", "1.2f"])
        params = Params(args)
    ...
    with pytest.raises(ValueError):
        args = parse_args(
            ["--popsize", "A=10", "B=10", "--prop-asym-carriers", "1.5", "2.5", "3.5"]
        )
        params = Params(args)
    ...
    args = parse_args(["--popsize", "A=10", "B=10", "--prop-asym-carriers", "A=1.2"])
    params = Params(args)
